...
AnkiJp is a fully offline React PWA for learning the 1,006 教育漢字 (joyo kanji) with Anki-style SM-2 SRS and quizzes. No backend; all data is bundled and SRS state lives in the browser's IndexedDB.
...
- `src/components/` — shared UI (StudyCard, SessionView, RatingButtons, KanjiDetail, Furigana, quiz views).
- `src/lib/` — all logic: `srs.ts` (SM-2), `db.ts` (IndexedDB via `idb`), `kanji.ts` (data access), `quiz.ts`, `stats.ts`, session hooks.
- `src/data/kanji.json` is the committed, generated dataset (1,006 entries). Don't edit it by hand — regenerate via the data pipeline below.

## Data pipeline (scripts/*.mjs)

One-off Node scripts that regenerate `src/data/kanji.json`:

- `npm run data:pipeline` runs the whole chain; `npm run data:verify` must pass 100% before committing.
- Network-dependent (`kanjiapi.dev`, scraping `jpdb.io`, Tatoeba dumps). Fetch scripts cache intermediates in `scripts/out/` (gitignored) and are resumable/rate-limited.
- Steps: `data:kyoiku` → `data:jpdb` → `data:tatoeba` → `data:mnemonics` → `data:build` → `data:verify`.
