  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between text-sm text-slate-600">
        <span>
          Question <span className="font-semibold text-slate-900">{index + 1}</span> of {total}
        </span>
        <span className="text-slate-400">{INSTRUCTION[question.mode]}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {question.options.map((option) => {
          let classes = 'border-slate-300 bg-white text-slate-800 hover:bg-slate-50 active:scale-95'
        ...
        <div className="flex flex-col items-center gap-3">
          <p
            className={`text-lg font-semibold ${
              selection === question.correct ? 'text-green-600' : 'text-red-600'
            }`}
          >
